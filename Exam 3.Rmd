...
where $y_2$=educ, the variable that is suspected to have endogeneity. Now, a regression for educ($y_2$) is made. The three IV's for educ is added in the regression:
...
\itemize{$E(v)=0$. The mean of the error term is 0, which indicates unbiasedness.}
\itemize{$Cov(x,v)=0$. This applies for all variables in the original model.}
\itemize{$Cov(z,v)=0$. The IV's should be independent of the error term.}
...
The null hypothesis is that all IV's are uncorrelated with $\u$. $nR^{2}_1 \sim \chi^2_q$, where $\q$ is the number of instrumental variables from outside the model minus the total number of endogenous explanatory variables. Hypothesis will be tested: 
##7. Perform the entire analysis again using only meduc and feduc as instruments. Does this change your conclusions?
We perform the 2SLS method again as in 3.5, but this time 
