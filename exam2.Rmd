...
## 4. Explain why it could be relevant to include $educ^2$ as an explanatory variable in the two models. Estimate the two models again with $educ^2$ included (along with its corresponding coefficient $\beta_5$). Briefly comment on the output, and perform the RESET test again.
If it is still assumed, that the model is misspecified, then it could be argued that including $educ^2$ could help correctly specify the model. Excluding the variable could lead to a bias on educ in the original model. Furthermore, by including $educ^2$, the model will be able to capture any quadratic forms. 
